...
class RegistryBrowser {
  public:
    ...
    explicit RegistryBrowser(boost::asio::io_context& io_context) : io_context_(io_context), timer_(io_context) {}

    /**
     * Start browsing for registries and nodes on the network.
     */
    void find_most_suitable_registry_async(
        const DiscoverMode discover_mode, const ApiVersion api_version, Callback callback
    ) {
        if (callback == nullptr) {
            RAV_ERROR("Callback is null");
            return;
        }
        ...
        timer_.once(k_default_timeout, [this, cb = std::move(callback)] {
  ...
  private:
    ...
    [[nodiscard]] std::optional<int> filter_and_get_pri(const dnssd::ServiceDescription& desc) const {
        ...
        if (api_proto->second != "http") {
            return false;  // TODO: Only http supported
        }
        ...
        if (api_auth->second != "false") {
            return false;  // TODO: Only no auth supported
        }

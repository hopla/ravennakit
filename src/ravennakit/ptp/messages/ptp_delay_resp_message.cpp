...
tl::expected<size_t, rav::output_stream::error> rav::ptp_delay_resp_message::write_to(output_stream& stream) const {
    const auto pos = stream.get_write_position();
    OK_OR_RETURN(receive_timestamp.write_to(stream));

...
TEST_CASE("ptp_request_response_delay_sequence") {
    SECTION("Test two-step sequence") {
        const auto t1 = rav::ptp_timestamp(10, 0);  // Sync send time
        const auto t2 = rav::ptp_timestamp(11, 0);  // Sync receive time
        const auto t3 = rav::ptp_timestamp(12, 0);  // Delay req send time
        const auto t4 = rav::ptp_timestamp(14, 0);  // Delay resp receive time
        ...
        rav::ptp_request_response_delay_sequence seq(sync_message);
        REQUIRE(seq.get_state() == rav::ptp_request_response_delay_sequence::state::awaiting_follow_up);

        seq.update(follow_up_message);
        REQUIRE(seq.get_state() == rav::ptp_request_response_delay_sequence::state::ready_to_be_scheduled);

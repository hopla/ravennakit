/*
 * Owllab License Agreement
 *
 * This software is provided by Owllab and may not be used, copied, modified,
 * merged, published, distributed, sublicensed, or sold without a valid and
 * explicit agreement with Owllab.
 *
 * Copyright (c) 2024 Owllab. All rights reserved.
 */

#pragma once

#include "ravennakit/containers/input_stream.hpp"

#include <ios>
#include <array>

namespace rav {

/**
 * Format for working with wav files.
 */
class wav_audio_format {
public:
    enum class format_code : uint16_t { pcm = 0x1, ieee_float = 0x3, alaw = 0x4, mulaw = 0x7, extensible = 0xfffe };

private:
    struct fmt_chunk {
        format_code audio_format;                           // A number indicating the WAVE format category of the file.
        uint16_t num_channels;                              // The number of channels represented in the waveform data.
        uint32_t sample_rate;                               // The sampling rate (in samples per second).
        uint32_t avg_bytes_per_sec;                         // The average number of bytes per second.
        uint16_t block_align;                               // The block alignment (in bytes) of the waveform data.
        uint16_t bits_per_sample;                           // Bits per sample.
        std::optional<uint16_t> cb_size;                    // The size of the extension (in bytes).
        std::optional<uint16_t> valid_bits_per_sample;      // Number of valid bits per sample.
        std::optional<uint32_t> channel_mask;               // Speaker position mask.
        std::optional<std::array<uint8_t, 16>> sub_format;  // GUID, including the data format code.

        void parse(input_stream& istream, uint32_t chunk_size);
    };

public:
    /**
     * A reader class which reads audio (meta)data from an input stream.
     */
    class reader {
      public:
        explicit reader(input_stream& istream);

        [[nodiscard]] format_code format() const;
        [[nodiscard]] uint16_t num_channels() const;
        [[nodiscard]] uint32_t sample_rate() const;

      private:
        size_t data_begin_ {};
        size_t data_size_ {};
        fmt_chunk fmt_chunk_;
    };

    /**
     * A writer class which writes audio (meta)data to an output stream.
     */
    class writer {
      public:
        explicit writer(std::ostream& istream);

      private:
    };
};

}  // namespace rav

...
rav::wrapping_uint<unsigned short> rav::rtp_packet::sequence_number_inc(const uint16_t value) {
    sequence_number_ += value;
...
rav::wrapping_uint<unsigned> rav::rtp_packet::timestamp_inc(const uint32_t value) {
    timestamp_ += value;
    return timestamp_;
}
